...
def test(FLAGS):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            # mcm = multilabel_confusion_matrix(label_list, pred_y, labels=[2, 1, 0])
            # print(mcm)
            # mcmtn = mcm[:, 0, 0]
            # mcmtp = mcm[:, 1, 1]
            # mcmfn = mcm[:, 1, 0]
            # mcmfp = mcm[:, 0, 1]
            # print(mcmtp/(mcmtp+mcmfn))
